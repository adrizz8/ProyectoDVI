import Phaser from 'phaser';
import NPC from './npc.js';

/**
 * Clase que representa el jugador del juego. El jugador se mueve por el mundo usando los cursores.
 * También almacena la puntuación o número de estrellas que ha recogido hasta el momento.
 */



export default class cafeteria_loco extends NPC {
    constructor(scene, player, x, y, texture,frame, message = null, onFinish = null, itemId = null,name) {
        super(scene, player, x, y, texture,frame, message , onFinish , itemId,name);

        this.ini_x=x;
        this.lastDirection='';
        
        this.frozen = false;
        
        this.body.moves = true;
        this.speed = 60;

       

        // Definición de animaciones direccionales
        const animsConfig = [
            { key: 'down', start: 0, end: 3 },
            { key: 'left', start: 4, end: 7 },
            { key: 'right', start: 8, end: 11 },
            { key: 'up', start: 12, end: 15 }
        ];

        animsConfig.forEach(cfg => {
            // Animación de caminar
            if (!this.scene.anims.exists(`walk6-${cfg.key}`)) {
                this.scene.anims.create({
                    key: `walk6-${cfg.key}`,
                    frames: this.scene.anims.generateFrameNumbers('npc3', { start: cfg.start, end: cfg.end }),
                    frameRate: 10,
                    repeat: -1
                });
            }
            // Animación de reposo (solo el primer frame de esa dirección)
            if (!this.scene.anims.exists(`idle6-${cfg.key}`)) {
                this.scene.anims.create({
                    key: `idle6-${cfg.key}`,
                    frames: this.scene.anims.generateFrameNumbers('npc3', { start: cfg.start, end: cfg.start }),
                    frameRate: 1,
                    repeat: -1
                });
            }
        });

        this.setDirection('right');

    }

    setDirection(dir) {
        this.lastDirection = dir;
        this.play(`idle6-${dir}`);
    }

    interact() {

        if (this.message != null) {
            this.freeze();
            this.player.freeze();
            this.say(this.message, () => {
                // Si el NPC nos tiene que dar un objeto, se procesa aquí
                this.receiveItem();
                if (this.onFinish) this.onFinish();
            });
        }
    }
    preUpdate(t, dt) {
        super.preUpdate(t, dt);

        dt/=10;
        const x=300;
        

        
        if (this.frozen) {
            this.body.setVelocity(0, 0);
            this.anims.stop();
            return;
        }
        let moving = false;

        if (this.lastDirection=='left') {

            if(Math.abs(this.player.y-this.y)<25&&this.x-this.player.x>=30&&this.x-this.player.x<=60){
                this.freeze();
                this.player.freeze();
                this.interact();
            }
            else{
                this.body.setVelocityX(-this.speed);
                this.body.setVelocityY(0);
                moving = true;
                
                if(this.x<this.ini_x){
                    this.x=this.ini_x;
                    this.lastDirection='right';
                }
            }
        }
        else if (this.lastDirection=='right') {

            if(Math.abs(this.player.y-this.y)<25&&this.x-this.player.x<=-30&&this.x-this.player.x>=-60){
                this.freeze();
                this.player.freeze();
                this.interact();
            }
            else{

            this.body.setVelocityX(this.speed);
            this.body.setVelocityY(0);
            moving = true;
            
                
                if(this.x>=this.ini_x+x){
                    this.x=this.ini_x+x;
                    this.lastDirection='left';
                }
            }
        } else {
            this.body.setVelocityX(0);
            this.body.setVelocityY(0);
        }

        // Reproducir la animación correspondiente: walk o idle según la última dirección
        const animState = moving ? 'walk6' : 'idle6';
        this.play(`${animState}-${this.lastDirection}`, true);
    
    }
    freeze(){
        this.play('idle6-'+this.lastDirection,true);
        this.frozen=true;
    }
    unfreeze() {
        this.frozen = false;
    }

}